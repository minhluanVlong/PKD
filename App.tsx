          ...
          <section className="lg:col-span-4 space-y-6 no-print">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl shadow-indigo-100/50 border border-white">
              ...
              <div className="space-y-4">
                ...
                <div className="relative group">
                  <label className="text-[10px] font-bold text-slate-400 uppercase ml-3 mb-1.5 block group-focus-within:text-indigo-500 transition-colors">Tên Điều dưỡng 3 (Mã 368)</label>
              ...
              <div className="space-y-6">
                ...
                <div ref={printRef} className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200 border border-slate-100 overflow-hidden w-full pdf-container">
                  <div id="print-header" className="p-10 hidden text-center">
                    <div className="flex justify-start items-start mb-10 text-left font-bold uppercase text-[11pt]">
                      <p>Bệnh viện Đa khoa khu vực Chợ Lách<br/>Khoa Nội - Nhi - Nhiễm</p>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-slate-50/50 border-b border-slate-100 text-slate-500 text-[10px]">
                          <th className="px-3 py-5 text-center font-black uppercase tracking-widest border-r border-slate-50 w-12">STT</th>
                          <th className="px-4 py-5 font-black uppercase tracking-widest border-r border-slate-50 min-w-[200px]">Bệnh nhân</th>
                          <th className="px-3 py-5 text-center font-black uppercase tracking-widest border-r border-slate-50 w-24">Y lệnh</th>
                          <th className="px-4 py-5 text-center font-black uppercase tracking-widest bg-blue-50/80 border-r border-blue-100 text-blue-700 w-28">Bắt đầu</th>
                          <th className="px-4 py-5 text-center font-black uppercase tracking-widest bg-emerald-50/80 border-r border-emerald-100 text-emerald-700 w-28">Kết thúc</th>
                          <th className="px-4 py-5 font-black uppercase tracking-widest border-r border-slate-50 w-40">Điều dưỡng</th>
                          <th className="px-3 py-5 text-center font-black uppercase tracking-widest border-r border-slate-50 w-20">Máy</th>
                          <th className="px-4 py-5 font-black uppercase tracking-widest">Ghi chú</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {schedule.map((row) => (
                          <tr key={`\${row.patientName}-\${row.doseNumber}`} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-3 py-4 text-center text-slate-400 border-r border-slate-50 font-medium">{row.stt}</td>
                            <td className="px-4 py-4 border-r border-slate-50">
                              <div className="flex items-center gap-2">
                                <span className="font-bold uppercase text-slate-800 text-[11px] truncate">{row.patientName}</span>
                                <span className={`px-1.5 py-0.5 rounded-[4px] text-[7px] font-black uppercase tracking-tighter shrink-0 \${row.doseNumber === 1 ? 'bg-indigo-50 text-indigo-500 border border-indigo-100' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>L{row.doseNumber}</span>
                              </div>
                            </td>
                            <td className="px-3 py-4 text-center border-r border-slate-50 text-slate-900 font-mono font-bold">{row.orderTime}</td>
                            <td className="px-4 py-4 text-center border-r border-blue-100 bg-blue-50/20 font-black text-blue-700 text-sm">{row.startTime}</td>
                            <td className="px-4 py-4 text-center border-r border-emerald-100 bg-emerald-50/20 font-black text-emerald-700 text-sm">{row.endTime}</td>
                            <td className="px-4 py-4 border-r border-slate-50 text-slate-700 font-semibold text-[11px] whitespace-nowrap">{row.nurseName}</td>
                            <td className="px-3 py-4 text-center border-r border-slate-50 font-mono font-black text-slate-900 bg-slate-50/20">{row.machineId}</td>
                            <td className="px-4 py-4 text-[10px] text-slate-400">{row.notes || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
